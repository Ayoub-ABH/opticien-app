...
def search_client():
    ...
    total_pages = max(1, (len(search_results) + rows_per_page - 1) // rows_per_page)
    update_table(search_results)  # Update the table with the search results
...
columns = ("ID", "N/P", "Date", "Docteur", "OD Puissance", "OG Puissance", "ADD Puissance", "Nature Verre", "Société", "Prix")
tree = ttk.Treeview(root, columns=columns, show="headings", height=5)  # Set height to 5 rows
...
tree = ttk.Treeview(root, columns=columns, show="headings", height=5)
    ...
    # Define smaller font for treeview text
tree_font = ('Arial', 8)  # Small font size
    
    # Set column headings and align the text to center
for col in columns:
        .heading(col, text=col)
        tree.column(col, anchor="center", width=100)  # Center-align text in the columns and set width
    
    # Apply the smaller font to all cells (items) in the treeview
    tree.tag_configure("small_font", font=tree_font)
