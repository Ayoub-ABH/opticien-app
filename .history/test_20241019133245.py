...
def open_add_client_window(parent):
    ...
    # Smaller font for labels only
    label_font = ('Arial', 2)  # Change the font size for labels to make them smaller
    
    for i, label in enumerate(labels):
        tk.Label(add_window, text=label, font=label_font).grid(row=i, column=0, padx=10, pady=5, sticky="w")  # Apply smaller font to labels
        entry = ttk.Entry(add_window)  # Keep the default font for the entry fields
        entry.grid(row=i, column=1, padx=10, pady=5)  # Keep padding as is
        entries[label] = entry
